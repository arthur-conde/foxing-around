...
exports.run = (client, message, args) => {
    ...
    if (!option || option == "true") {
        ...
        for (i in monitorconfig.voicechannels) {
            // only push monitorconfig.voicechannels entries for channels on the guild the command is invoked in
    ...
    if (option == "remove" || option == "-") {
        ...
        for (i in monitorconfig.voicechannels) {
            if (monitorconfig.voicechannels[i].voiceID == monitorVoice) {
                // when monitorVoice matches a .voiceID entry remove it, write file, send confirmation
